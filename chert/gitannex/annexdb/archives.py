...
def _archive_filename_query(session, oclass, attr, rarfiles=False):
    likezip = getattr(oclass, attr).like('%.zip')
    likerar = getattr(oclass, attr).like('%.rar')
    if rarfiles:
        return session.query(oclass).filter(or_(likezip, likerar))
    return session.query(oclass).filter(likezip)

def _archive_filename_filter(query, oclass, attr, rarfiles=False):
    likezip = getattr(oclass, attr).like('%.zip')
    likerar = getattr(oclass, attr).like('%.rar')
    if rarfiles:
...
def _archive_filename_query(session, oclass, attr, rarfiles=False):
    ...
    return _archive_filename_filter(query, oclass, attr, rarfiles=rarfiles)

    
def get_archive_files_query(session, rarfiles=False):
    return _archive_filename_query(session,
                                   AnnexFile, 'name', rarfiles=rarfiles)
    



def get_archive_files_new(query, rarfiles=False):
    q =  _archive_filename_filter(query, AnnexFile, 'name',
                                  rarfiles=rarfiles)
    return q.all()
...
def make_archive_entry_zipfile(session, entry):
...
def insert_archive_entry_archive_entry(session, archive_entry_id, entry):
    pass
